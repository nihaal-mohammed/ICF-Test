...
try:
    ...
    try:
        ...
        def search_chroma(query: str):
            ...
            try:
                ...
                if doc_count == 0:
                    ...
                    return [
                        [
                            "No documents found in database. Please run your data pipeline first."
                        ]
                    ]
                ...
                results = collection.query(
                    query_embeddings=query_embeddings, n_results=10
                )

                print(
                    f"[✓] Search results for query '{query[:50]}...': {len(results['documents'][0]) if results['documents'] else 0} found."
                )
        ...
        search_chroma_function = search_chroma
        print(
            "✅ Created search_chroma function directly with Backend.vector_db_pipline import"
        )

    except ImportError as e2:
        print(f"❌ Could not import Backend.vector_db_pipline: {e2}")
        print(
            "💡 Please ensure your Backend/vector_db_pipline.py file exists and is importable"
        )
    ...
    try:
        ...
        if relevant_chunks and len(relevant_chunks) > 0:
            ...
            for chunk_list in relevant_chunks:
                if isinstance(chunk_list, list):
                    ...
                    valid_chunks = [
                        ...
                        if chunk.strip()
                        and not chunk.startswith("No search functionality")
                        and not chunk.startswith("Error searching")
                        and not chunk.startswith("No documents found")
                    ]
                    all_chunks.extend(valid_chunks)
                else:
                    if chunk_list.strip() and not chunk_list.startswith(
            ...
            if all_chunks:
                ...
                dynamic_context = "\n\n".join(
                    [f"Context {i+1}: {chunk}" for i, chunk in enumerate(top_chunks)]
                )
...
async def root():
    return {
        "message": "Islamic Center of Frisco RAG System is running!",
        "status": "online",
    }
